...
abstract class TmpProtectionBase
{
    ...
    protected function init($level_id)
    {
        ...
        if (is_numeric($level_id)) {
            $result = $wpdb->get_row(
                $wpdb->prepare(
                    "SELECT * FROM {$wpdb->prefix}tmp_memberships WHERE id = %d",
                    $level_id
                )
            );
        } else {
            $result = $wpdb->get_row(
                $wpdb->prepare(
                    "SELECT * FROM {$wpdb->prefix}tmp_memberships WHERE md5(id) = %s",
                    $level_id
                )
            );
        }
    ...
    public function save()
    {
        ...
        $wpdb->update($wpdb->prefix . "tmp_memberships", $data, array('id' => $this->owning_level_id));
    }
    ...
    public function in_parent_categories($id)
    {
        ...
        foreach ($terms as $term) {
            ...
            if (in_array($term->parent, $this->categories)) {
                return true;
            }
        }
        return false;
