...
?>
<div class="wrap" id="tmp-profile-page" type="edit">

	<p>会員を編集します。</p>
	<style>
	</style>

	<div class="tmp-member-edit-registed">
		<h4>登録済みデータ</h4>
		<table class="tmp-member-edit-table">
			<tbody>
				<tr>
					<th scope="row">ID</th>
				...
				<tr>
					<th scope="row">ユニークコード</th>
					<td><?php if (isset($unique_code)) echo esc_html($unique_code); ?></td>
				</tr>
				<tr>
					<th scope="row">表示名</th>
					<td><?php if (isset($display_name)) echo esc_html($display_name); ?></td>
				</tr>
				<tr>
					<th scope="row">名前</th>
					<td><?php if (isset($name)) echo esc_html($name); ?></td>
				</tr>
				<tr>
					<th scope="row">メールアドレス</th>
					<td><?php if (isset($email)) echo esc_html($email); ?></td>
				</tr>
				<tr>
					<th scope="row">所持チケット一覧</th>
					<td>
						<?php
						if (isset($ticket_list_serialized) && !empty($ticket_list_serialized) && !empty($array_ticket_mst)) {
							$ticket_list = unserialize($ticket_list_serialized, ['allowed_classes' => false]);
							if (!is_array($ticket_list)) {
								$ticket_list = [];
							}
							$html = '';
							foreach ($array_ticket_mst as $val) {
				...
				<tr>
					<th scope="row">会員レベル</th>
					<td><?php if (isset($memberships_name)) echo esc_html($memberships_name); ?></td>
				</tr>
				<tr>
					<th scope="row">会員レベル判定日時</th>
					<td>
						<?php
						if (isset($memberships_check_date)) {
							if ($memberships_check_date == '0000-00-00 00:00:00') {
								echo " - ";
							} else {
								echo esc_html($memberships_check_date);
							}
						}
						?>
					</td>
				...
				<tr>
					<th scope="row">会員登録日時</th>
					<td><?php if (isset($created)) echo esc_html($created); ?></td>
				</tr>
			</tbody>
		</table>
	</div>
		...
		<table class="form-table">
			<tbody>
				<tr>
					...
					<td>
						<input class="regular-text" name="unique_code" type="text" value="<?php if (isset($unique_code)) echo esc_attr($unique_code); ?>" id="unique_code" required="true" readonly="readonly" />
					</td>
				</tr>
			</tbody>
		</table>
	...
	<!-- DB編集用フォーム -->
	<form action="" method="post" name="tmp-edit-user" id="tmp-edit-user" class="validate">
		<input name="action" type="hidden" value="edituser" />
		<input name="unique_code" id="unique_code" type="hidden" value="" />
		<?php
		//フォーム送信元チェック用
		wp_nonce_field('edit_tmp_user_admin_end', '_wpnonce_edit_tmp_user_admin_end')
		?>
		<table class="form-table">
			<tbody>
				<tr class="tmp-registration-email-row">
		...
		<?php submit_button("更新", 'primary', 'edittmpuser', true, array('id' => 'edittmpusersub')); ?>
	</form>
</div>
<script>
	var json_ticket_mst = JSON.parse('<?php echo wp_json_encode($array_ticket_mst, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP); ?>');
	...
	jQuery(document).ready(function($) {
		...
		$('#tmp-get-user-from-api #apitmpuser').click(function() {
			...
			$("#tmp-edit-user input#unique_code").val(unique_code);
			return false;
		});
				...
				.done(function(data) {
					...
					if (data.result) {
						if (data.hUser.User.name_sei) {
							$("#tmp-edit-user input#name_sei").val(data.hUser.User.name_sei);
						}
						if (data.hUser.User.name_mei) {
							$("#tmp-edit-user input#name_mei").val(data.hUser.User.name_mei);
						}
						if (data.hUser.User.mail) {
							$("#tmp-edit-user input#email").val(data.hUser.User.mail);
						}
						return true;
					}
					return false;
			...
			$.ajax(
				...
				.done(function(data) {
					...
					if (data.id) {
						...
						$("#tmp-edit-user #memberships_id").val(membership_level_id);
						return true;
					}
				})
				// 検索失敗時には、その旨をダイアログ表示
